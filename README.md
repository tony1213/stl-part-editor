...
- **橙色 = 这一面从外面看得见**：选中件上直接标出它露出来的那些面，配合页脚的「露出 N 面」，
  ...
  橙色成片就是删了会在外表面留豁口
- **标出外露面**（<kbd>E</kbd>）：全局版。把所有待删零件的外露面一起标橙。
  和下面那个开关配合用：**取消「透视待删零件」+ 打开这个 = 剔除后的干净结果上，
  橙色直接标出外表面哪几处会缺**——这是决定"这批到底能不能删"最直接的一眼
- **透视待删零件**（<kbd>H</kbd>）：勾上＝待删零件透过外壳以半透明红色显出来，看清里面塞了什么；
  取消＝预览剔除后的干净结果
...
- <kbd>F</kbd> 重新聚焦当前选中件，<kbd>Delete</kbd> 删除当前选中件
